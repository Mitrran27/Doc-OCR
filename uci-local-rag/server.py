...
# def list_conversations(doc_id: Optional[str] = Query(default=None)):
#     return conversation_store.list_conversations(doc_id=doc_id)
...
#     if conv is None:
#         raise HTTPException(404, f"Unknown conversation_id: {conversation_id}")
#     return conv
...
#     if conversation_id is None or conversation_store.get_conversation(conversation_id) is None:
#         conversation_id = conversation_store.create_conversation(request.doc_id)
...
#     conversation_store.add_message(conversation_id, role="user", text=request.query)
#     conversation_store.add_message(
#         conversation_id, role="assistant", text=result["answer"],
#         sources=result["sources"], images=images,
#     )
...
Endpoints (matching the required contract):
    ...
    GET  /documents/        -> lists indexed documents with doc_id & metadata
    POST /chat/             -> query + doc_id -> retrieve from Chroma -> generate via Qwen
    GET  /chat/history      -> full prior chat log for a doc_id, so page2 can restore it on load
...
def chat(request: ChatRequest):
    ...
    # Persist both sides of the exchange so page2 can restore this
    # conversation on a later visit (via "Open" on page1 -> chat.html?doc_id=...)
    # without needing any separate "load history" action from the user.
    chat_store.append_message(request.doc_id, role="user", text=request.query)
...
def chat_history(doc_id: str = Query(...)):
    doc = doc_store.get_document(doc_id)
    if doc is None:
        raise HTTPException(404, f"Unknown doc_id: {doc_id}")
    return chat_store.get_history(doc_id)
